...
7. **ML2-5b Bridge affects ordering (when trusted)**
   ...
   - **Recommended next pipeline move (k=4 geometry):** Same snapshot **`source-snapshot-20260329-170012`**, same embedding **`v1-title-abstract-1536-cleantext-r2`**. `python -m pipeline.cli cluster-works --embedding-version v1-title-abstract-1536-cleantext-r2 --cluster-version <new label e.g. kmeans-l2-v0-cleantext-r2-k4> --corpus-snapshot-version source-snapshot-20260329-170012 --cluster-count 4`. Then **`embedding-coverage ... --cluster-version <cv> --fail-on-gaps`**, **`GET /api/v1/clusters/{cluster_version}/inspect`**, new **`ranking-run`** with **`--cluster-version <cv>`** and **`--bridge-weight-for-family-bridge 0.0`**, **ML2-5a-style** qualitative bridge review (signal density, reasons, list plausibility). **Only after** that, consider **isolated** positive weights on the **new** artifact and repeat overlap checks.
