...
```
   ...
   - **Verdict (first k-review, pre-full embed / pre-r2 cluster):** ML2-5a passed as an engineering milestone, but **ML2-5b was premature** while bridge signal was sparse and the head of the list stayed emerging-shaped. **Mojibake** in several titles is a **separate corpus hygiene** issue, not a bridge-logic issue.
   - **Follow-up (`ml2-5a-qual-r2-k3-20260405`, `ranking_run_id` `rank-17658d0f74`, same snapshot after full `v1-title-abstract-1536-cleantext-r2` embed + `kmeans-l2-v0-cleantext-r2-k3` cluster):** In bridge-family **top 20**, every row had non-null `signals.bridge` and structural-boundary `reason_short` (fallback no longer dominated the head). Sparsity is **much reduced**; `final_score` is still unchanged because bridge weight remains **0.0**, so ordering at the head may still look heuristic/emerging-shaped. At least one **mojibake** survivor remains (e.g. `Cross-modal` title noise). **Interpretation:** ML2-5a is materially stronger; **ML2-5b is now worth a cautious, isolated experiment** (new `ranking_version`, small bridge-only weight, inspectable diff vs this baseline) - not default production weighting until overlap and evaluation checks pass. **Update (2026-04-05):** That **first** ML2-5b sweep on **this k3 artifact** is **complete** (**0.06**, **0.10**); **no further weight increases** on this geometry (**step 7**). Next: **higher-k geometry** experiments (**k=4** recorded below; **k=5** next), not more weight on k3.
   - **What next for ML:**
     - Increase **embedding coverage** so more ranked bridge candidates can receive cluster-backed bridge scores. Use `python -m pipeline.cli embedding-coverage --embedding-version <label> [--corpus-snapshot-version ...] [--fail-on-gaps]` to audit gaps; after **`cluster-works`**, add `--cluster-version <cv>` to see `missing_cluster_assignment` (included works with no `clusters` row for that label) and use `--fail-on-gaps` to require both `missing_embedding=0` and `missing_cluster_assignment=0`. Run `embed-works` **without** `--limit` for full snapshot coverage (limit is for smoke tests only). `ranking-run --cluster-version` prints a **stderr warning** when any included work still lacks that embedding version. Re-run **`cluster-works`** after embeddings are complete so every embedded work gets an assignment.
     ...
     - **Re-run clustering** and the same bridge review once the embedded slice is broader; after r2+k3, signal density supported **isolated ML2-5b weight experiments** (see ML2-5b step: sweep now **closed** on k3; next change **k / signal / corpus**, not higher **w** on the same geometry).
     - **Handoff runbook (any machine; example snapshot `source-snapshot-20260329-170012`):** In `services/pipeline`, set `DATABASE_URL`. **(1)** `python -m pipeline.cli embedding-coverage --embedding-version <ev> --corpus-snapshot-version <snap>` (optional `--fail-on-gaps`). **(2)** `repair-works-text --corpus-snapshot-version <snap> --dry-run`. **Labels:** if `rows_changed=0`, keep the same `embedding_version` and use a **new** `cluster_version` (e.g. `...-k3-r2`); if `rows_changed>0`, run `repair-works-text` without `--dry-run`, then mint a **new** `embedding_version` (e.g. `...-cleantext-r2`) and matching **new** `cluster_version` (e.g. `kmeans-l2-v0-cleantext-r2-k3`) so vectors match cleaned text. **(3)** `embed-works` with **no** `--limit`. **(4)** `embedding-coverage ... --fail-on-gaps` until `missing_embedding=0`. **(5)** `cluster-works` for that `embedding_version` + fresh `cluster_version` (use **`--cluster-count K`** for k-means k). **(5b)** `embedding-coverage --embedding-version <ev> --corpus-snapshot-version <snap> --cluster-version <cv> --fail-on-gaps` until `missing_cluster_assignment=0` (errors if there is no matching `clustering_runs` row). **(6)** `GET /api/v1/clusters/{cluster_version}/inspect` (API must be up). **(7)** New `ranking-run` with **`--bridge-weight-for-family-bridge` default 0.0** + bridge qualitative / ML2-5a-style API check; **positive bridge weight** only after a **new** geometry passes distinctness review (overlap / list quality). `embed-works` needs the configured embedding provider (e.g. OpenAI) in env.

7. **ML2-5b Bridge affects ordering (when trusted)**
   - **Before first weighted run:** On the baseline ranking (e.g. `rank-17658d0f74`), count non-null `bridge_score` across **all** bridge-family rows (not only top 20); compute **bridge top-10 vs emerging top-10** work-id overlap (Jaccard or raw count). Very high overlap -> keep weight tiny or defer.
   - **Isolated weight experiments (harness):** New `ranking_version` per run; same `corpus_snapshot_version`, `embedding_version`, and `cluster_version` as the ML2-5a control under test. **`ranking-run --bridge-weight-for-family-bridge W`** (default **`0.0`**; range **[0.0, 0.25]**). Persisted: **`config_json.family_weights.bridge.bridge`**, **`clustering_artifact.bridge_weight_in_final_score`**, **`bridge_reason_mode`**. Emerging/undercited **`final_score`** unchanged. **Do not** edit checked-in `FAMILY_WEIGHTS` for runs; CLI resolves per execution.
   - **Validation gates:** No NaN/inf in scores; diff report: bridge `final_score` and ranks vs baseline; overlap metric after weighting; evaluation surfaces still truthful. **Not** the ML2-5a SQL `final_score` invariance gate for bridge family when weight > 0.
   - **Recorded 0.06 (2026-04-05):** **`ml2-5b-exp-r2-k3-w006-20260405`**, **`rank-16c1cfb490`**. Harness + honest API copy validated.
   - **Recorded 0.06 / 0.10 overlap (API `limit=10`, snapshot `source-snapshot-20260329-170012`, control `ml2-5a-qual-r2-k3-20260405`):** bridge vs emerging **top-10** overlap **8/10** at **0.06** and again at **0.10**; **bridge top-10 membership** **unchanged** vs ML2-5a (reorder only within the same ten papers).
   - **Recorded 0.10 (2026-04-05):** **`ml2-5b-exp-r2-k3-w010-20260405`**, **`rank-cf04ae30c6`**. Same overlap outcome as 0.06.
   - **Decision: weight sweep closed on this setup:** For **`v1-title-abstract-1536-cleantext-r2`** + **`kmeans-l2-v0-cleantext-r2-k3`** + **cluster-boundary ratio** bridge signal, **do not** push higher weights (**no 0.15, 0.20, ...**). The question is answered: **weight alone did not produce a distinct bridge head** (overlap and membership unchanged vs ML2-5a control). **Production / default:** keep **`ranking-run` default `0.0`** and public pins on **ML2-5a-style** runs until a **new** cluster geometry or signal is validated.
   - **Product / copy:** If bridge is surfaced now, describe it as **structurally informed reordering** within a shared pool, **not** a clearly separate recommendation family yet.
   - **Scientific boundary:** Close the **weight** sweep first (**done**); **then** run **cluster/signal/corpus** experiments - not parallel weight tuning on a frozen geometry.
   - **k=4 follow-up (`ml2-5a-qual-r2-k4-20260405`, `ranking_run_id` `rank-b39d9e0d4f`, cluster `kmeans-l2-v0-cleantext-r2-k4`):** Bridge vs emerging **top-10** overlap remained **8/10**; **bridge top-10 membership** was **identical** to **`ml2-5a-qual-r2-k3-20260405`**. **No** distinctness gain on this metric; **skip** weighted ML2-5b on k4; move to **higher-k** geometry.
   - **k=5 follow-up (`ml2-5a-qual-r2-k5-20260405`, cluster `kmeans-l2-v0-cleantext-r2-k5`):** Inspect showed a smaller catch-all and new **c004** (three-paper task/challenge slice); ranking + overlap vs **`ml2-5a-qual-r2-k3-20260405`**: bridge vs emerging **top-10** overlap still **8/10**; **bridge top-10 membership** still **identical** to k3.
   - **k=6 follow-up (`ml2-5a-qual-r2-k6-20260405`, `ranking_run_id` `rank-83976f1097`, cluster `kmeans-l2-v0-cleantext-r2-k6`):** Inspect: catch-all **14** works, stable diversity cluster, same **2-paper** outlier pair as k5, **c004** unchanged vs k5, new **c005** (jazz/piano/MIDI corpora). **Numeric `signals.bridge` moved** for many rows vs k3/k5, but **`final_score`** at the head is still driven by citation/topic (bridge weight **0**). Overlap vs k3: bridge/emerging **top-10** still **8/10**; **bridge top-10 set** still **identical** to k3.
   - **Decision: k-means k sweep closed (this embed + snapshot):** For **`v1-title-abstract-1536-cleantext-r2`** on **`source-snapshot-20260329-170012`**, varying **k** in **{4,5,6}** did **not** change bridge **top-10 membership** or **8/10** emerging overlap vs the **k3** baseline. **Do not** chase **k=7+** here; next levers are **more works**, **different bridge feature**, or **semantic in `final_score`**, not further k-means **k** on this slice.
   - **API inspection note:** `GET .../ranked` JSON includes explicit **`"bridge": null`** when the score is unset (`response_model_exclude_none=False` on that route). Re-serializing with **Windows PowerShell `ConvertTo-Json`** can **drop null-valued properties** on nested objects, so a missing `signals.bridge` key in pasted output is not proof the API omitted it. Prefer a direct property read (e.g. `(Invoke-RestMethod ...).items[n].signals.bridge`) or raw response bytes before assuming a reason/signal mismatch.
