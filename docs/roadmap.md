...
7. **ML2-5b Bridge affects ordering (when trusted)**
   ...
   - **First experiment (isolated):** New `ranking_version` label (e.g. `ml2-5b-exp-r2-k3-w006-20260405`); same `corpus_snapshot_version`, `embedding_version`, and `cluster_version` as the ML2-5a control. Use **`ranking-run --bridge-weight-for-family-bridge W`** (default `0.0`; try **0.06** then **0.10** if needed - bounded by CLI to **[0.0, 0.25]**). Do **not** edit checked-in `FAMILY_WEIGHTS`; the run resolves weights per execution and persists **`config_json.family_weights.bridge.bridge`**, **`clustering_artifact.bridge_weight_in_final_score`** (float), and **`clustering_artifact.bridge_reason_mode`** (`structural_zero_weight` vs `structural_weighted`). **`reason_short`** for structural bridge rows matches the resolved weight. Re-run `ranking-run`, compare bridge ordering vs baseline; emerging/undercited **`final_score`** should be unchanged (they do not use bridge signal). Example: `python -m pipeline.cli ranking-run --ranking-version ml2-5b-exp-r2-k3-w006-20260405 --embedding-version v1-title-abstract-1536-cleantext-r2 --cluster-version kmeans-l2-v0-cleantext-r2-k3 --corpus-snapshot-version source-snapshot-20260329-170012 --bridge-weight-for-family-bridge 0.06`
   - **Validation gates:** No NaN/inf in scores; diff report: bridge `final_score` and ranks vs baseline; overlap metric again after weighting; evaluation surfaces still truthful. **Not** the ML2-5a SQL "final_score invariant" gate - scores **should** move for bridge family only.
