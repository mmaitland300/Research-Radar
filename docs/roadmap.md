...
7. **ML2-5b Bridge affects ordering (when trusted)**
   ...
   - **Recommended next pipeline move (k=5 geometry):** Same snapshot **`source-snapshot-20260329-170012`**, same embedding **`v1-title-abstract-1536-cleantext-r2`**. `python -m pipeline.cli cluster-works --embedding-version v1-title-abstract-1536-cleantext-r2 --cluster-version kmeans-l2-v0-cleantext-r2-k5 --corpus-snapshot-version source-snapshot-20260329-170012 --cluster-count 5 --max-iterations 20`. Gate: `python -m pipeline.cli embedding-coverage --embedding-version v1-title-abstract-1536-cleantext-r2 --cluster-version kmeans-l2-v0-cleantext-r2-k5 --corpus-snapshot-version source-snapshot-20260329-170012 --fail-on-gaps`. Inspect: `GET /api/v1/clusters/kmeans-l2-v0-cleantext-r2-k5/inspect?sample_per_cluster=5`. **Continue** to a zero-weight **`ranking-run`** (new `ranking_version`) **only if** inspect looks **meaningfully** different: the large catch-all **breaks up** believably, smaller coherent groups **stay** coherent, geometry could **plausibly** change bridge-head membership. If k5 is the same structure with **unhelpful** fragmentation only, try **k=6** once (`kmeans-l2-v0-cleantext-r2-k6`); **stop** escalating k if that also fails. **Only after** a geometry passes review, consider **isolated** positive weights and repeat overlap checks.
