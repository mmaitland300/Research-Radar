# Manual label dataset (ml-label-dataset-v14)
...
- `docs/audit/manual-review/bridge_shadow_pilot_rank-5a7efa5ca3_v1_labeled_2026-06-02.csv`
- `docs/audit/manual-review/bridge_top_ranked_rank-83787b91ef_v1_labeled_2026-06-01.csv`
...
Rows from worksheets with `review_pool_variant=ml_bridge_shadow_pilot_audit` are bridge-family shadow-pilot disagreement labels from `rank-5a7efa5ca3`. They preserve the full work-id keyed sidecar row under nested `bridge_shadow_pilot_context`, including `current_family_rank`, `hybrid_rank`, `disagreement_bucket`, `ml_probability`, `bridge_score`, `hybrid_score`, `ml_rank_pct`, and `bridge_score_rank_pct`. These labels are audit-only evidence and do not authorize Bridge serving.

### Bridge shadow-pilot v1 ingest

- **Rows appended:** 60
- **Legacy rows:** copied from v13 with deterministic `bridge_recommendable` backfilled where needed.
- **Final dataset rows:** 897
- **Ranking run / family:** `rank-5a7efa5ca3` / `bridge`
- **Review pool:** `ml_bridge_shadow_pilot_audit`
- **Raw relevance distribution:** `{'acceptable': 5, 'good': 48, 'irrelevant': 4, 'miss': 3}`
- **Raw bridge-like distribution:** `{'no': 26, 'partial': 15, 'yes': 19}`
- **bridge_recommendable positives / negatives:** 34 / 26
- **Disagreement buckets:** `{'demoted_by_hybrid': 20, 'high_bridge_score_low_ml': 10, 'high_ml_low_bridge_score': 10, 'promoted_by_hybrid': 20}`
- **Overlap count with v13:** 32 appended observations; overlaps are reported, not deduped.
...
- **Next step:** `refresh_bridge_shadow_pilot_readiness_matrix_v11`.
...
- **Duplicate `paper_id` count** (papers with more than one retained row): 158
- **Conflicting raw label groups** (same `paper_id`, same label field, multiple distinct non-empty values): 266
...
- **Derived target conflict count:** 100
...
Machine-readable export: regenerate via `python -m pipeline.cli ml-label-dataset-v14-bridge-shadow-pilot-ingest --base-dataset docs/audit/ml-label-dataset-v13.json --blank-worksheet docs/audit/manual-review/bridge_shadow_pilot_rank-5a7efa5ca3_v1_blank.csv --labeled-worksheet docs/audit/manual-review/bridge_shadow_pilot_rank-5a7efa5ca3_v1_labeled_2026-06-02.csv --context-sidecar docs/audit/manual-review/bridge_shadow_pilot_rank-5a7efa5ca3_v1_context.json --output <path>.json`.
