# Manual label dataset (ml-label-dataset-v13)
...
- **Duplicate `paper_id` count** (papers with more than one retained row): 129
- **Conflicting raw label groups** (same `paper_id`, same label field, multiple distinct non-empty values): 222
...
- **Derived target conflict count:** 78
