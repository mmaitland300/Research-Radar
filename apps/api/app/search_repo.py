...
from app.scores_repo import resolve_ranked_run_context
...
class SearchResponseRow:
    ...
    resolved_ranking_run_id: str | None = None
    resolved_ranking_version: str | None = None
    resolved_corpus_snapshot_version: str | None = None


class SearchRunContextNotFoundError(LookupError):
    """Raised when family-filtered search requires a ranking run that cannot be resolved."""
    ...
    *,
    ...
    with psycopg.connect(database_url_from_env(), row_factory=dict_row) as conn:
        ...
        if family_hint is not None:
            resolved_run = resolve_ranked_run_context(
                conn,
            ...
            if resolved_run is None:
                raise SearchRunContextNotFoundError(
        ...
        if resolved_run is not None and family_hint is not None:
            where_clauses.append(
                """
                EXISTS (
                    SELECT 1
                    ...
                    WHERE ps_filter.work_id = w.id
                      AND ps_filter.ranking_run_id = %s
                      AND ps_filter.recommendation_family = %s
                )
                """
            )
            params.append(resolved_run.ranking_run_id)
    ...
    items = [
        SearchResultRow(
            ...
            preview=(
                _normalize_preview(str(row["abstract_preview"]))
                if row["abstract_preview"] is not None
                else None
            ),
    ...
    return SearchResponseRow(
        ...
        resolved_filters=SearchResolvedFiltersRow(
            ...
            ranking_run_id=ranking_run_id if family_hint is not None else None,
            ranking_version=ranking_version if family_hint is not None else None,
        ),
        items=items,
        resolved_ranking_run_id=resolved_run.ranking_run_id if resolved_run is not None else None,
        resolved_ranking_version=resolved_run.ranking_version if resolved_run is not None else None,
        resolved_corpus_snapshot_version=(
            resolved_run.corpus_snapshot_version if resolved_run is not None else None
        ),
    )
