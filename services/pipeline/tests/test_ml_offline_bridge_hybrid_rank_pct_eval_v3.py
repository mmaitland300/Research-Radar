"""Tests for ml_offline_bridge_hybrid_rank_pct_eval_v3."""
...
from pipeline.ml_offline_bridge_hybrid_eval_v3 import _validate_prerequisite_sensitivity_artifact
...
def _make_shadow_rows() -> list[dict]:
    ...
    for bucket, pos, neg in (
        ("promoted_by_hybrid", 14, 6),
        ("demoted_by_hybrid", 8, 12),
        ("high_ml_low_bridge_score", 10, 0),
        ("high_bridge_score_low_ml", 2, 8),
    ):
        for _ in range(pos):
...
def test_prerequisite_rejects_not_ready() -> None:
    ...
    with pytest.raises(MLOfflineBridgeHybridRankPctEvalV3Error, match="ready_for_offline_hybrid_eval"):
...
def test_bridge_score_coverage_aborts() -> None:
    ...
    ml_pct = {r["work_id"]: 0.5 for r in rows}
    bridge_pct = {r["work_id"]: 0.5 for r in rows}
    bridge_raw = {
        r["work_id"]: r["bridge_shadow_pilot_context"]["bridge_score"] for r in rows
    }
    with pytest.raises(MLOfflineBridgeHybridRankPctEvalV3Error, match="bridge_score coverage too low"):
        _build_scored_rows(
            rows,
            oof_by_work_id=oof,
            ml_rank_pct_by_token=ml_pct,
            bridge_rank_pct_by_token=bridge_pct,
            bridge_score_by_token=bridge_raw,
        )


def test_slice_only_scope_by_default(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    with patch(
        "pipeline.ml_offline_bridge_hybrid_rank_pct_eval_v3.sha256_file",
        return_value="abc123",
    ):
    ...
    assert payload["rank_percentile_scope"] == SCOPE_LABELED_SLICE
    assert payload["pool_candidate_count"] == EXPECTED_SHADOW_ROWS
    assert payload["ml_probability_source_for_rank_pct"] == "oof_sensitivity_C0_001"
...
def test_full_pool_scope_when_db_and_shadow_pilot(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    rows = _make_shadow_rows()
    candidates = []
    for i in range(528):
        token = rows[i]["work_id"] if i < len(rows) else f"W{2000000000 + i}"
        candidates.append(
            {
                "work_id_int": 1000 + i,
                "work_id_token": token,
                "openalex_id": f"https://openalex.org/{token}",
                "bridge_score": 0.5 + (i % 50) * 0.01,
            }
        )
    shadow_path = tmp_path / "shadow-pilot.json"
    ...
    fake_probs = {c["work_id_token"]: 0.01 * (i + 1) for i, c in enumerate(candidates)}
    for row in rows:
        fake_probs[row["work_id"]] = 0.9 if row[TARGET] else 0.1

    with patch(
        "pipeline.ml_offline_bridge_hybrid_rank_pct_eval_v3.sha256_file",
        return_value="abc123",
    ), patch(
        "pipeline.ml_offline_bridge_hybrid_rank_pct_eval_v3._score_pool_ml_probabilities",
    ...
    assert payload["rank_percentile_scope"] == SCOPE_FULL_POOL
    assert payload["pool_candidate_count"] == 528
    assert payload["ml_probability_source_for_rank_pct"] == "frozen_v3_C0_001_full_fit"
...
def test_markdown_contains_scope_section(tmp_path: Path) -> None:
    paths = _write_fixture(tmp_path)
    with patch(
        "pipeline.ml_offline_bridge_hybrid_rank_pct_eval_v3.sha256_file",
        return_value="abc123",
    ...
    md = markdown_from_ml_offline_bridge_hybrid_rank_pct_eval_v3(payload)
    assert "rank_percentile_scope" in md
    assert "rank-percentile" in md.lower()
...
def test_happy_path_payload_structure(tmp_path: Path) -> None:
    ...
    assert payload["labeled_row_scores"][0]["ml_rank_pct"] is not None


def test_module_has_no_top_level_psycopg_import() -> None:
    import pipeline.ml_offline_bridge_hybrid_rank_pct_eval_v3 as mod
    ...
    joined = "\n".join(top_level)
    assert "import psycopg" not in joined
