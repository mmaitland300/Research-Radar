"""Offline diagnostic scorer for bridge_recommendable on the combined v13 bridge slice.

Consumes both the bridge negative-mining labels (70 rows, v12) and the bridge
top-ranked validation labels (30 rows, v13) for a total of 100 rows covering the
actual live top-Bridge surface as well as the buried hard-negative candidates.

Key decision signal: does the model correctly assign lower probability to the 12
bridge_like=no papers in the top-20 live Bridge ranking than to the 8 yes/partial
papers that are already there?  See ``top_20_live_bridge_comparison`` in the output.
...
EXPECTED_HARD_NEGATIVE_COUNT = 37
...
ARTIFACT_TYPE = "ml_offline_bridge_recommendable_scorer_v2"
FIT_MODE = "bridge_combined_slice_diagnostic_v1"
...
SAMPLE_REASON_TOP_RANKED = "bridge_top_ranked"
EXPECTED_TOP_20_COUNT = 20
EXPECTED_TOP_20_POSITIVE_COUNT = 8
EXPECTED_TOP_20_NEGATIVE_COUNT = 12
...
class MLOfflineBridgeRecommendableScorerV2Error(Exception):
...
def _load_json_object(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise MLOfflineBridgeRecommendableScorerV2Error(f"failed to load {label} JSON {path}: {exc}") from exc
    if not isinstance(payload, dict):
        raise MLOfflineBridgeRecommendableScorerV2Error(f"{label} JSON must be an object: {path}")
    return payload
...
def _parse_vector(value: Any) -> list[float]:
    if isinstance(value, str):
        try:
            ...
            raise MLOfflineBridgeRecommendableScorerV2Error(f"embedding vector is not valid JSON: {exc}") from exc
    else:
        parsed = value
    if not isinstance(parsed, (list, tuple)):
        raise MLOfflineBridgeRecommendableScorerV2Error("embedding vector must be an array")
    out: list[float] = []
    for item in parsed:
        number = _as_float(item)
        if number is None:
            raise MLOfflineBridgeRecommendableScorerV2Error("embedding vector contains non-finite/non-numeric value")
        out.append(number)
    return out
...
def _execute_select(cur: Any, sql: str, params: Sequence[Any] | None = None) -> Any:
    ...
    if not lowered.startswith("select"):
        raise MLOfflineBridgeRecommendableScorerV2Error("DB safety violation: SQL must start with SELECT")
    if WRITE_SQL_RE.search(lowered):
        raise MLOfflineBridgeRecommendableScorerV2Error("DB safety violation: SQL contains write/DDL verb")
...
def _validate_readiness_matrix(
    ...
    *,
    ...
    if not isinstance(prov, dict):
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix missing provenance object")
    if prov.get("label_dataset_version") != "ml-label-dataset-v13":
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix must point at ml-label-dataset-v13")
    if prov.get("label_dataset_sha256") != label_dataset_sha256:
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix label_dataset_sha256 does not match label dataset")
    groups = readiness_payload.get("groups")
    if not isinstance(groups, list):
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix missing groups array")
    target_group = None
    for group in groups:
    ...
    if target_group is None:
        raise MLOfflineBridgeRecommendableScorerV2Error(
    ...
    if not isinstance(readiness, dict):
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix target group missing readiness object")
    if readiness.get("enough_for_diagnostic_auc") is not True:
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix target group is not diagnostic-AUC ready")
    if readiness.get("enough_for_tiny_baseline") is not True:
        raise MLOfflineBridgeRecommendableScorerV2Error("readiness matrix target group is not tiny-baseline ready")
    return target_group
...
def _validate_embeddings_provenance(payload: dict[str, Any]) -> int:
    ...
    if not isinstance(metadata, dict):
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance missing metadata object")
    checks = {
    ...
    if not isinstance(result, dict):
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance missing embedding_result object")
    if result.get("status") != "succeeded":
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance status must be succeeded")
    if result.get("full_snapshot_embedding_coverage") is not True:
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance full_snapshot_embedding_coverage must be true")
    dim = result.get("embedding_dimensions")
    if not isinstance(dim, int) or isinstance(dim, bool) or dim <= 0:
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance embedding_dimensions must be positive integer")
    coverage = payload.get("coverage")
    if not isinstance(coverage, dict):
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance missing coverage object")
    if coverage.get("embedded_work_count") != EXPECTED_EMBEDDED_WORK_COUNT:
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance embedded_work_count must be 528")
    if coverage.get("missing_embedding_count") != 0:
        raise MLOfflineBridgeRecommendableScorerV2Error("embeddings provenance missing_embedding_count must be 0")
    # Tests may use tiny fixture dimensions; real committed artifact must be 1536.
    return dim
...
def _slice_rows(label_payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(rows, list):
        raise MLOfflineBridgeRecommendableScorerV2Error("label dataset missing rows array")
    out = [
        row
        for row in rows
...
def _validate_slice(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if checks["positive_count"] != EXPECTED_POSITIVE_COUNT or checks["negative_count"] != EXPECTED_NEGATIVE_COUNT:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"bridge_recommendable counts mismatch: {checks['positive_count']}/{checks['negative_count']}; "
            f"expected {EXPECTED_POSITIVE_COUNT}/{EXPECTED_NEGATIVE_COUNT}"
        )
    if checks["bridge_like_label"] != {"no": 47, "partial": 35, "yes": 18}:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"bridge_like_label distribution mismatch: {checks['bridge_like_label']}"
        )
    if hard_negative_count != EXPECTED_HARD_NEGATIVE_COUNT:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"hard_negative_count={hard_negative_count}; expected {EXPECTED_HARD_NEGATIVE_COUNT}"
        )
    if leak_count != 0:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"bridge-like positive relevance leak_count={leak_count}; expected 0"
        )
    if duplicate_row_ids:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"duplicate row_id values in slice: {duplicate_row_ids[:5]}"
        )
    if duplicate_work_ids:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"duplicate work_id values in slice: {duplicate_work_ids[:5]}"
        )
    if dconf != 0:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"derived_target_conflict_count={dconf}; expected 0"
        )
    return checks


def _internal_work_ids(rows: list[dict[str, Any]]) -> list[int]:
    """Read internal_work_id from the top-level field (present on both v12 and v13 rows)."""
    out: list[int] = []
    for row in rows:
    ...
    if len(set(out)) != EXPECTED_SLICE_ROWS:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            "internal_work_id values are not unique within slice"
        )
    return out
...
def _load_slice_embeddings_select_only(
    ...
    *,
    ...
    if len(internal_work_ids) != EXPECTED_SLICE_ROWS:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"embedding query must be limited to exactly {EXPECTED_SLICE_ROWS} slice work_ids"
        )
    sql = """
    ...
    if missing or extra or len(out) != EXPECTED_SLICE_ROWS:
        raise MLOfflineBridgeRecommendableScorerV2Error(
            f"embedding coverage mismatch: found={len(out)}, missing={missing[:10]}, extra={extra[:10]}"
        )
    return out
...
def _stratified_metrics(
    ...
    *,
    ...
    """Ranking metrics for a sub-slice identified by row indices into the full slice."""
    sub_scores = [oof_prob[i] for i in indices]
...
def _top_20_comparison(
    rows: list[dict[str, Any]],
    oof_prob: list[float],
) -> dict[str, Any]:
    """Compute the key decision signal: does the model score yes/partial top-20 rows
    higher than the no rows that currently occupy the live Bridge top-20?"""
    top20_idx = [i for i, r in enumerate(rows) if r.get("sample_reason") == SAMPLE_REASON_TOP_RANKED]
    if len(top20_idx) != EXPECTED_TOP_20_COUNT:
        return {
            "status": "unexpected_count",
            "expected": EXPECTED_TOP_20_COUNT,
            "found": len(top20_idx),
        }

    pos_idx = [i for i in top20_idx if rows[i].get(TARGET) is True]
    neg_idx = [i for i in top20_idx if rows[i].get(TARGET) is False]
    pos_probs = [oof_prob[i] for i in pos_idx]
    neg_probs = [oof_prob[i] for i in neg_idx]

    mean_pos = float(statistics.fmean(pos_probs)) if pos_probs else None
    mean_neg = float(statistics.fmean(neg_probs)) if neg_probs else None

    # Pairwise correct-ordering fraction over (positive, negative) pairs
    total_pairs = len(pos_probs) * len(neg_probs)
    correct_pairs = sum(1 for p in pos_probs for n in neg_probs if p > n)
    correct_fraction = correct_pairs / total_pairs if total_pairs > 0 else None

    verdict: str
    if correct_fraction is None:
        verdict = "not_applicable"
    elif correct_fraction > 0.75:
        verdict = "model_strongly_demotes_top20_bridge_like_no_papers"
    elif correct_fraction > 0.5:
        verdict = "model_partially_demotes_top20_bridge_like_no_papers"
    else:
        verdict = "model_fails_to_demote_top20_bridge_like_no_papers"

    per_row = sorted(
        [
            {
                "row_id": str(rows[i].get("row_id") or ""),
                "work_id": rows[i].get("work_id"),
                "family_rank": rows[i].get("family_rank"),
                "bridge_like_label": rows[i].get("bridge_like_label"),
                "bridge_recommendable": rows[i].get(TARGET),
                "oof_probability": oof_prob[i],
            }
            for i in top20_idx
        ],
        key=lambda r: r.get("family_rank") or 999,
    )

    return {
        "status": "ok",
        "n_top20": len(top20_idx),
        "positive_count": len(pos_idx),
        "negative_count": len(neg_idx),
        "mean_oof_probability_positive": mean_pos,
        "mean_oof_probability_negative": mean_neg,
        "mean_prob_gap_positive_minus_negative": (
            round(mean_pos - mean_neg, 6) if mean_pos is not None and mean_neg is not None else None
        ),
        "pairwise_correct_ordering_fraction": correct_fraction,
        "correct_pairs": correct_pairs,
        "total_pairs": total_pairs,
        "verdict": verdict,
        "interpretation": (
            "Fraction of (bridge_like yes/partial, bridge_like no) pairs in the top-20 live Bridge "
            "rows where the model assigns higher probability to the yes/partial paper. "
            ">0.75 means the model credibly re-ranks; >0.5 means partial signal; <=0.5 means the model "
            "fails to improve on the current live order and needs more labels or features."
        ),
        "per_row": per_row,
    }
...
def _learned_cv(
    ...
    *,
    ...
    """Run 5-fold stratified CV; return the aggregate result and the flat OOF probability list."""
    ...
    if any(math.isnan(p) for p in oof_prob):
        raise MLOfflineBridgeRecommendableScorerV2Error("OOF probabilities did not cover all rows")
    ...
    # Build stratified sub-slice metrics using the same OOF probabilities
    neg_mining_idx = [
        i for i, r in enumerate(rows)
        if r.get("review_pool_variant") == "ml_bridge_negative_mining_audit"
    ]
    top_ranked_idx = [
        i for i, r in enumerate(rows)
        if r.get("review_pool_variant") == "ml_bridge_top_ranked_validation_audit"
    ]
    top_20_idx = [i for i, r in enumerate(rows) if r.get("sample_reason") == SAMPLE_REASON_TOP_RANKED]

    stratified = [
        _stratified_metrics(rows, oof_prob, indices=list(range(len(rows))), label="all_100_rows"),
        _stratified_metrics(rows, oof_prob, indices=neg_mining_idx, label="negative_mining_slice_70_rows"),
        _stratified_metrics(rows, oof_prob, indices=top_ranked_idx, label="top_ranked_validation_slice_30_rows"),
        _stratified_metrics(rows, oof_prob, indices=top_20_idx, label="top_20_live_bridge_rows"),
    ]
    ...
    result = {
        ...
        "stratified_oof_metrics": stratified,
        "oof_predictions": [
            {
...
def _baseline_winner_readout(learned_cv: dict[str, Any], heuristic_arms: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "readout": (
            f"{winner} has the highest ROC AUC on this offline diagnostic combined slice. "
...
def _metadata_input_paths(label_payload: dict[str, Any], *, label_dataset_path: Path) -> list[dict[str, Any]]:
    ...
    for ingest_key in ("bridge_negative_mining_v1_ingest", "bridge_top_ranked_v1_ingest"):
...
def build_ml_offline_bridge_recommendable_scorer_v2_payload(
    conn: psycopg.Connection,
    *,
    ...
    for path in (label_path, readiness_path, embeddings_path):
        if not path.is_file():
            raise MLOfflineBridgeRecommendableScorerV2Error(f"required input not found: {path}")
    ...
    if label_payload.get("dataset_version") != "ml-label-dataset-v13":
        raise MLOfflineBridgeRecommendableScorerV2Error("label dataset must be ml-label-dataset-v13")
    ...
    learned_cv, oof_prob = _learned_cv(rows, vectors_by_work, random_seed=random_seed)
    heuristic_arms = {
    ...
    top_20_comparison = _top_20_comparison(rows, oof_prob)
    ...
    return {
        ...
        "metadata": {
            ...
            "label_distributions": {
                ...
                "review_pool_variant_counts": slice_counts["review_pool_variant_counts"],
            },
        ...
        "top_20_live_bridge_comparison": top_20_comparison,
        ...
        "recommended_next_stage": (
            "bridge_scorer_serving_candidate_review"
            if top_20_comparison.get("verdict", "").startswith("model_strongly_demotes")
            else "collect_more_labels_or_feature_work_before_bridge_serving"
        ),
    }


def markdown_from_ml_offline_bridge_recommendable_scorer_v2(payload: dict[str, Any]) -> str:
    meta = payload["metadata"]
    learned = payload["evaluation"]["learned_cv"]["aggregate_oof"]
    stratified = payload["evaluation"]["learned_cv"].get("stratified_oof_metrics", [])
    heur = payload["evaluation"]["heuristic_arms"]
    winner = payload.get("baseline_winner_readout_cv_auc", {})
    top20 = payload.get("top_20_live_bridge_comparison", {})
    slice_n = meta["slice_counts"]["row_count"]
    lines = [
        "# Offline bridge recommendable scorer v2 — combined slice diagnostic",
        "",
        "Offline diagnostic model for `bridge_recommendable` trained on both bridge label slices: "
        "the negative-mining worksheet (70 rows) and the top-ranked validation worksheet (30 rows). "
        "This is not validation and not a serving change.",
        "",
        "## Slice",
        "",
        f"- Rows: {slice_n} (70 negative-mining + 30 top-ranked validation)",
    ...
    if top20.get("status") == "ok":
        lines += [
            f"- Top-20 rows: {top20.get('n_top20')} (currently-positive: {top20.get('positive_count')}, currently-negative: {top20.get('negative_count')})",
            f"- Mean OOF probability — positive: {top20.get('mean_oof_probability_positive'):.4f}, negative: {top20.get('mean_oof_probability_negative'):.4f}",
            f"- Probability gap (pos − neg): {top20.get('mean_prob_gap_positive_minus_negative')}",
            f"- Pairwise correct-ordering fraction: {top20.get('pairwise_correct_ordering_fraction'):.3f} ({top20.get('correct_pairs')}/{top20.get('total_pairs')} pairs)",
            f"- **Verdict: {top20.get('verdict')}**",
            "",
            top20.get("interpretation", ""),
        ]
    else:
        lines.append(f"- Status: {top20.get('status')}")
    lines += [
        "",
        "## Heuristic Arms",
        "",
        f"| arm | status | coverage | ROC AUC | AP | P@10 |",
        "|---|---:|---:|---:|---:|---:|",
        *[
            (
                f"| `{name}` | {arm.get('status')} | {arm.get('non_null_score_count')}/{slice_n} | "
...
def write_ml_offline_bridge_recommendable_scorer_v2(
    conn: psycopg.Connection,
    *,
    label_dataset_path: Path,
    readiness_matrix_path: Path,
    ...
    if markdown_path is not None:
        ...
        markdown_path.write_text(
            markdown_from_ml_offline_bridge_recommendable_scorer_v2(payload), encoding="utf-8"
        )
    return payload


def run_ml_offline_bridge_recommendable_scorer_v2_cli(
    *,
    ...
    with psycopg.connect(dsn) as conn:
        write_ml_offline_bridge_recommendable_scorer_v2(
            conn,
            label_dataset_path=label_dataset_path,
            readiness_matrix_path=readiness_matrix_path,
            embeddings_provenance_path=embeddings_provenance_path,
