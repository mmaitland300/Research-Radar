"""Bounded production-scoped production default/API/user-visible pilot.

This runner re-reads approved production sources, enables the shadow runtime
only inside a process-scoped drill, builds an in-process audit-only response
shape probe, writes isolated audit artifacts, and advances the bundle only
after the rev23 checks pass. It never binds an HTTP server, calls production API
routes, emits responses to users, or writes production tables/config.
"""
...
from pipeline.ml_shadow_scorer_production_scoped_shadow_bundle import (
    ...
    PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_ID_PREFIX,
...
EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT = EXPECTED_LIVE_READ_ONLY_PILOT_ROW_COUNT
PRODUCTION_DEFAULT_API_USER_VISIBLE_API_SURFACE = "/api/v1/recommendations/ranked"
PRODUCTION_DEFAULT_API_USER_VISIBLE_PROBE_LIMIT = 20


class MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(Exception):
...
def _default_pilot_run_id(generated_at: str) -> str:
    ...
    if compact.endswith("Z"):
        compact = compact[:-1] + "Z"
    return f"{PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_ID_PREFIX}-{RANKING_RUN_ID}-{compact}"
...
def _shadow_row_export_rows(shadow_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for rank, row in enumerate(shadow_rows, start=1):
        out.append(
            {
                "audit_only": True,
                "pilot_surface": PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_SURFACE,
                "shadow_rank": rank,
...
def _current_output_rows(runtime_rows: Sequence[Mapping[str, Any]], *, limit: int) -> list[dict[str, Any]]:
    rows = sorted(
        runtime_rows,
        key=lambda row: (-float(row.get("final_score") or 0.0), str(row.get("canonical_openalex_work_id") or "")),
    )
    return [
        {
            "rank": rank,
...
def _would_be_shadow_output_rows(shadow_rows: Sequence[Mapping[str, Any]], *, limit: int) -> list[dict[str, Any]]:
    return [
        {
            "rank": rank,
            "canonical_openalex_work_id": row.get("canonical_openalex_work_id"),
...
def _build_production_default_api_user_visible_probe(
    *,
    runtime_rows: Sequence[Mapping[str, Any]],
    shadow_rows: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    return {
        "api_surface": PRODUCTION_DEFAULT_API_USER_VISIBLE_API_SURFACE,
        "family": FAMILY,
        "ranking_run_id": RANKING_RUN_ID,
        "limit": PRODUCTION_DEFAULT_API_USER_VISIBLE_PROBE_LIMIT,
        "bridge_surface_included": False,
        "in_process_audit_only_probe": True,
        "current_output_read_only_probe_performed": True,
        "would_be_shadow_scorer_output_built": True,
        "user_visible_response_emitted_to_users": False,
        "production_default_changed": False,
        ...
        "current_output_top_20": _current_output_rows(
            runtime_rows,
            limit=PRODUCTION_DEFAULT_API_USER_VISIBLE_PROBE_LIMIT,
        ),
        "would_be_shadow_scorer_output_top_20": _would_be_shadow_output_rows(
            shadow_rows,
            limit=PRODUCTION_DEFAULT_API_USER_VISIBLE_PROBE_LIMIT,
        ),
    }
...
def _input_provenance(bundle: Mapping[str, Any], live_source_reads: Mapping[str, Any]) -> dict[str, Any]:
    ...
    audit_pilot = execution.get("pilot_run", {}) if isinstance(execution, Mapping) else {}
    return {
        ...
        "previous_audit_artifact_pilot_run_id": audit_pilot.get("pilot_run_id")
        if isinstance(audit_pilot, Mapping)
        else None,
...
def _production_default_api_user_visible_scope(bundle: Mapping[str, Any]) -> dict[str, Any]:
    authorization = bundle.get("authorization", {})
    return {
        "production_default_api_user_visible_grant_decision": deepcopy(
            authorization.get("production_default_api_user_visible_grant_decision")
        ),
        "production_default_api_user_visible_granted_scope": deepcopy(
            authorization.get("production_default_api_user_visible_granted_scope")
        ),
        "bounded_production_default_api_user_visible_pilot_only": True,
    ...
    *,
    ...
    return {
        ...
        "signals_emitted": [
            ...
            "production default/API/user-visible in-process probe",
        ],
        ...
        "component_coverage": {
            "complete": len(runtime_rows) == EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT
            and len(shadow_rows) == EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT,
        ...
        "rank_displacement_audit_only": "bounded_production_default_api_user_visible_prod_scoped_pilot",
        "production_default_api_user_visible_probe": deepcopy(dict(probe)),
    }
...
def _write_pilot_artifacts(
    *,
    ...
    production_default_api_user_visible_scope: Mapping[str, Any],
    production_default_api_user_visible_probe: Mapping[str, Any],
    incomplete_coverage: Mapping[str, Any],
    preflight: Mapping[str, Any],
    pilot: Mapping[str, Any],
    postflight: Mapping[str, Any],
    ...
    if pilot_dir.exists() and any(pilot_dir.iterdir()):
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
    ...
    manifest = {
        "artifact_type": (
            "ml_shadow_scorer_production_scoped_shadow_production_default_api_user_visible_pilot_manifest"
        ),
        ...
        "production_default_api_user_visible_scope": deepcopy(dict(production_default_api_user_visible_scope)),
        "production_default_api_user_visible_probe": deepcopy(dict(production_default_api_user_visible_probe)),
        "incomplete_coverage_drill": deepcopy(dict(incomplete_coverage)),
        "identity": {
            "ranking_run_id": RANKING_RUN_ID,
            "ranking_version": PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_RANKING_VERSION,
            "family": FAMILY,
            "candidate_pool_work_set_sha256": CANDIDATE_POOL_WORK_SET_SHA256,
            "corpus_snapshot_version": CORPUS_SNAPSHOT_VERSION,
            "embedding_version": EMBEDDING_VERSION,
        ...
        "component_coverage": {
            ...
            "expected_row_count": EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT,
        },
    }
    observability = _observability_summary(
        ...
        probe=production_default_api_user_visible_probe,
    )
...
def _build_pass_fail(
    *,
    ...
    probe = production_default_api_user_visible_probe
    checks = {
        "production_default_api_user_visible_grant_slices_present": isinstance(
            authorization.get("production_default_api_user_visible_grant_decision"),
            Mapping,
        )
        ...
        == EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT,
        "runtime_row_count_528": len(runtime_rows) == EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT,
        ...
        and pilot.get("shadow_row_count") == EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT,
        ...
        "bounded_api_surface_probe_performed": probe.get("api_surface") == PRODUCTION_DEFAULT_API_USER_VISIBLE_API_SURFACE
        and probe.get("in_process_audit_only_probe") is True
        and probe.get("current_output_read_only_probe_performed") is True,
        ...
        and probe.get("http_server_bound") is False
        ...
        and production_default_api_user_visible_scope.get("production_default_allowed") is False
        and production_default_api_user_visible_scope.get("api_web_changes_allowed") is False
        and production_default_api_user_visible_scope.get("user_visible_ranking_changed") is False,
        ...
        "isolated_artifact_count_expected": write_counts.get(ISOLATED_PROD_SCOPED_AUDIT_ARTIFACTS) == 4,
        "expected_files_recorded": [record.get("relative_path") for record in files_written]
        == list(PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_EXPECTED_FILES),
        "live_flag_and_read_only_chain_still_valid": authorization.get("prod_scoped_shadow_live_execution_authorized")
        is True
        and authorization.get("prod_scoped_shadow_flag_enablement_authorized") is True
        and isinstance(execution, Mapping)
        and execution.get("prod_scoped_shadow_live_read_only_pilot_passed") is True
        and execution.get("prod_scoped_shadow_live_execution_pilot_passed") is True
        and execution.get("prod_scoped_shadow_flag_enablement_pilot_passed") is True,
        "global_execution_authorization_false": production_default_api_user_visible_scope.get(
            "prod_scoped_shadow_execution_authorized"
        )
        is False
        and authorization.get("prod_scoped_shadow_execution_authorized") is False,
        "plan_flag_authorized_now_false": _plan_flag_authorized_now_false(bundle),
    ...
    missing = [name for name in PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_PASS_FAIL_CHECKS if name not in checks]
    if missing:
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "missing production default/API/user-visible pass/fail checks: " + ", ".join(missing)
        )
    failed = [
        name for name in PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_PASS_FAIL_CHECKS if not checks.get(name)
    ]
    return {
        "overall_passed": not failed,
        "checks": {name: checks[name] for name in PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_PASS_FAIL_CHECKS},
        "failed_checks": failed,
        "forbidden_nonzero_write_counts": forbidden_nonzero,
    }


def run_ml_shadow_scorer_production_scoped_shadow_production_default_api_user_visible_pilot(
    *,
    bundle_path: Path,
    ...
    if not confirm_production_default_api_user_visible_pilot:
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "--confirm-production-default-api-user-visible-pilot is required before bounded production default/API/user-visible pilot"
        )
    ...
    try:
        ...
        if not run_id.startswith(f"{PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_RUN_ID_PREFIX}-"):
            raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
                "pilot_run_id must use prod-output prefix"
            )
        ...
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(str(exc)) from exc
    ...
    try:
        ...
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(str(exc)) from exc
    if bundle.get("recommended_next_stage") != POST_PRODUCTION_DEFAULT_API_USER_VISIBLE_GRANT_NEXT_STAGE:
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "bundle recommended_next_stage must be run_production_scoped_online_shadow_production_default_api_user_visible_pilot_v1"
        )
    execution = bundle.get("execution", {})
    existing_ids = {
    ...
    if run_id in existing_ids:
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "pilot_run_id must differ from proof, harness, audit-artifact, live read-only, live execution, and flag enablement pilot run ids"
        )
    ...
    try:
        ...
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            f"production default/API/user-visible pilot database unavailable: {type(exc).__name__}: {exc}"
        ) from exc
    try:
        try:
    ...
    if (
        ...
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "production default/API/user-visible ranking_version must match approved production candidate ranking version"
        )
    ...
    if (
        pilot.get("status") != "succeeded_test_only"
        or pilot.get("shadow_row_count") != EXPECTED_PRODUCTION_DEFAULT_API_USER_VISIBLE_PILOT_ROW_COUNT
    ):
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "pilot runtime did not succeed with 528 rows"
        )
    ...
    production_default_api_user_visible_scope = _production_default_api_user_visible_scope(bundle)
    production_default_api_user_visible_probe = _build_production_default_api_user_visible_probe(
        runtime_rows=runtime_rows,
        shadow_rows=[row for row in shadow_rows if isinstance(row, Mapping)],
    )
    try:
        files_written, observability, write_counts_payload = _write_pilot_artifacts(
            ...
            production_default_api_user_visible_scope=production_default_api_user_visible_scope,
            production_default_api_user_visible_probe=production_default_api_user_visible_probe,
            incomplete_coverage=incomplete_coverage,
            preflight=preflight,
            pilot=pilot,
            postflight=postflight,
    ...
    if not pass_fail["overall_passed"]:
        raise MLShadowScorerProductionScopedShadowProductionDefaultAPIUserVisiblePilotError(
            "production default/API/user-visible pilot failed checks: " + ", ".join(pass_fail["failed_checks"])
        )
    ...
    pilot_slice = {
        ...
        "production_default_api_user_visible_scope": deepcopy(production_default_api_user_visible_scope),
        "runtime_drill": {
        ...
        "production_default_api_user_visible_probe": deepcopy(production_default_api_user_visible_probe),
        "files_written": files_written,
    ...
    try:
        updated_bundle = apply_production_scoped_shadow_production_default_api_user_visible_pilot_run(
            bundle,
            pilot_slice,
            generated_at=generated,
        )
        if updated_bundle.get("authorization") != bundle.get("authorization"):
            raise MLShadowScorerProductionScopedShadowBundleError(
                "production default/API/user-visible pilot wrapper must preserve authorization section"
            )
        ...
        for key, before_value in before_execution.items():
            if after_execution.get(key) != before_value:
                raise MLShadowScorerProductionScopedShadowBundleError(
                    f"production default/API/user-visible pilot wrapper must preserve execution.{key}"
                )
        verify_ml_shadow_scorer_production_scoped_shadow_bundle_payload(
            updated_bundle,
            repo_root=root,
