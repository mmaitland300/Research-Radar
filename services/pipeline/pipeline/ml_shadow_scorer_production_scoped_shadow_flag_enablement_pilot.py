"""Bounded production-scoped online shadow flag enablement pilot.

This runner re-reads approved production sources, enables the runtime flag only
inside a process-scoped drill, restores the environment afterward, writes
isolated audit artifacts, and advances the bundle only after rev19 checks pass.
"""
...
from pipeline.ml_shadow_scorer_production_scoped_shadow_bundle import (
    ...
    FLAG_ENABLEMENT_PILOT_RUN_ID_PREFIX,
...
EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT = EXPECTED_LIVE_READ_ONLY_PILOT_ROW_COUNT


class MLShadowScorerProductionScopedShadowFlagEnablementPilotError(Exception):
...
def _default_pilot_run_id(generated_at: str) -> str:
    ...
    if compact.endswith("Z"):
        compact = compact[:-1] + "Z"
    return f"{FLAG_ENABLEMENT_PILOT_RUN_ID_PREFIX}-{RANKING_RUN_ID}-{compact}"
...
def _shadow_row_export_rows(shadow_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for rank, row in enumerate(shadow_rows, start=1):
        out.append(
            {
                "audit_only": True,
                "pilot_surface": FLAG_ENABLEMENT_PILOT_RUN_SURFACE,
                "shadow_rank": rank,
...
def _observability_summary(
    *,
    ...
    return {
        ...
        "component_coverage": {
            "complete": len(runtime_rows) == EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT
            and len(shadow_rows) == EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT,
        ...
        "rank_displacement_audit_only": "bounded_flag_enablement_prod_scoped_pilot",
    }
...
def _flag_enablement_scope(bundle: Mapping[str, Any]) -> dict[str, Any]:
    authorization = bundle.get("authorization", {})
    return {
        "flag_enablement_grant_decision": deepcopy(authorization.get("flag_enablement_grant_decision")),
        "flag_enablement_granted_scope": deepcopy(authorization.get("flag_enablement_granted_scope")),
        "bounded_flag_enablement_pilot_only": True,
...
def _write_pilot_artifacts(
    *,
    ...
    flag_enablement_scope: Mapping[str, Any],
    incomplete_coverage: Mapping[str, Any],
    preflight: Mapping[str, Any],
    pilot: Mapping[str, Any],
    postflight: Mapping[str, Any],
    ...
    if pilot_dir.exists() and any(pilot_dir.iterdir()):
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
    ...
    manifest = {
        "artifact_type": "ml_shadow_scorer_production_scoped_shadow_flag_enablement_pilot_manifest",
        ...
        "flag_enablement_scope": deepcopy(dict(flag_enablement_scope)),
        "incomplete_coverage_drill": deepcopy(dict(incomplete_coverage)),
        "identity": {
            "ranking_run_id": RANKING_RUN_ID,
            "ranking_version": FLAG_ENABLEMENT_PILOT_RUN_RANKING_VERSION,
            "family": FAMILY,
            "candidate_pool_work_set_sha256": CANDIDATE_POOL_WORK_SET_SHA256,
            "corpus_snapshot_version": CORPUS_SNAPSHOT_VERSION,
            "embedding_version": EMBEDDING_VERSION,
        ...
        "component_coverage": {
            ...
            "expected_row_count": EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT,
        },
    }
...
def _build_pass_fail(
    *,
    ...
    checks = {
        "flag_enablement_grant_slices_present": isinstance(authorization.get("flag_enablement_grant_decision"), Mapping)
        ...
        == EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT,
        "runtime_row_count_528": len(runtime_rows) == EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT,
        ...
        and pilot.get("shadow_row_count") == EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT,
        ...
        == list(FLAG_ENABLEMENT_PILOT_RUN_EXPECTED_FILES),
        "live_execution_chain_still_valid": authorization.get("prod_scoped_shadow_live_execution_authorized") is True
        and bundle.get("execution", {}).get("prod_scoped_shadow_live_execution_pilot_passed") is True,
        ...
        and flag_enablement_scope.get("production_default_allowed") is False
        and flag_enablement_scope.get("api_web_changes_allowed") is False
        and flag_enablement_scope.get("user_visible_ranking_changed") is False,
        "global_execution_authorization_false": flag_enablement_scope.get("prod_scoped_shadow_execution_authorized")
        is False
    ...
    missing = [name for name in FLAG_ENABLEMENT_PILOT_RUN_PASS_FAIL_CHECKS if name not in checks]
    if missing:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "missing flag enablement pass/fail checks: " + ", ".join(missing)
        )
    failed = [name for name in FLAG_ENABLEMENT_PILOT_RUN_PASS_FAIL_CHECKS if not checks.get(name)]
    return {
        "overall_passed": not failed,
        "checks": {name: checks[name] for name in FLAG_ENABLEMENT_PILOT_RUN_PASS_FAIL_CHECKS},
        "failed_checks": failed,
        "forbidden_nonzero_write_counts": forbidden_nonzero,
    }


def run_ml_shadow_scorer_production_scoped_shadow_flag_enablement_pilot(
    *,
    bundle_path: Path,
    ...
    if not confirm_flag_enablement_pilot:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "--confirm-flag-enablement-pilot is required before bounded flag enablement pilot"
        )
    ...
    try:
        ...
        if not run_id.startswith(f"{FLAG_ENABLEMENT_PILOT_RUN_ID_PREFIX}-"):
            raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
                "pilot_run_id must use prod-flag-enable prefix"
            )
        if "harness" in run_id:
            raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError("pilot_run_id must not contain harness")
        ...
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(str(exc)) from exc
    ...
    if not isinstance(bundle, Mapping):
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError("bundle JSON must be an object")
    if bundle.get("execution", {}).get("prod_scoped_shadow_flag_enablement_pilot_executed") is True:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "flag enablement pilot run has already been filed"
        )
    try:
        verify_ml_shadow_scorer_production_scoped_shadow_bundle_payload(
            bundle,
        ...
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(str(exc)) from exc
    if bundle.get("recommended_next_stage") != POST_FLAG_ENABLEMENT_GRANT_NEXT_STAGE:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "bundle recommended_next_stage must be run_production_scoped_online_shadow_flag_enablement_pilot_v1"
        )
    execution = bundle.get("execution", {})
    existing_ids = {
    ...
    if run_id in existing_ids:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "pilot_run_id must differ from proof, harness, audit-artifact, live read-only, and live execution pilot run ids"
        )
    ...
    try:
        ...
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(str(exc), code=exc.code) from exc
    except Exception as exc:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            f"flag enablement pilot database unavailable: {type(exc).__name__}: {exc}"
        ) from exc
    try:
        try:
    ...
    if live_source_reads.get("ranking_run", {}).get("ranking_version") != FLAG_ENABLEMENT_PILOT_RUN_RANKING_VERSION:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "flag enablement ranking_version must match approved production candidate ranking version"
        )
    ...
    if preflight.get("status") != "skipped_runtime_disabled":
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError("preflight runtime must be disabled")
    if pilot.get("status") != "succeeded_test_only" or pilot.get("shadow_row_count") != EXPECTED_FLAG_ENABLEMENT_PILOT_ROW_COUNT:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError("pilot runtime did not succeed with 528 rows")
    if postflight.get("status") != "skipped_runtime_disabled":
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError("postflight runtime must be disabled")
    ...
    flag_enablement_scope = _flag_enablement_scope(bundle)
    try:
        files_written, observability, write_counts_payload = _write_pilot_artifacts(
            ...
            flag_enablement_scope=flag_enablement_scope,
            incomplete_coverage=incomplete_coverage,
            preflight=preflight,
            pilot=pilot,
            postflight=postflight,
    ...
    if not pass_fail["overall_passed"]:
        raise MLShadowScorerProductionScopedShadowFlagEnablementPilotError(
            "flag enablement pilot failed checks: " + ", ".join(pass_fail["failed_checks"])
        )
    ...
    pilot_slice = {
        ...
        "flag_enablement_scope": deepcopy(flag_enablement_scope),
        "runtime_drill": {
    ...
    try:
        updated_bundle = apply_production_scoped_shadow_flag_enablement_pilot_run(
            bundle,
            pilot_slice,
            generated_at=generated,
        )
